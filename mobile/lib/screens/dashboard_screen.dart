...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final unreadNotifs = notifications.where((n) => n['is_read'] == false).length;
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildSectionHeader('Volume Insights', LucideIcons.barChart2),
                    const SizedBox(height: 16),
                    _buildVolumeGrid(),
                    const SizedBox(height: 32),
                    ...
                    _buildRevenueCards(),
                    const SizedBox(height: 32),
                    _buildSectionHeader('Site Performance', LucideIcons.factory),
                    const SizedBox(height: 16),
                    _buildSiteList(),
  ...
  Widget _buildAppBar() {
    return SliverAppBar(
      ...
      elevation: 0,
      title: const Text('MilkFlow Admin', style: TextStyle(color: Color(0xFF1E293B), fontWeight: FontWeight.bold)),
      actions: [
        _buildIconButton(
          icon: LucideIcons.bell,
          count: _unreadNotifications,
          onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const NotificationsScreen())).then((_) => _loadData()),
        ),
        _buildIconButton(
          icon: LucideIcons.messageSquare,
          count: _unreadMessages,
          onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const ChatScreen())).then((_) => _loadData()),
        ),
  ...
  Widget _buildWelcomeHeader() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Hello, ${_userProfile?['username'] ?? 'User'}', style: TextStyle(color: Colors.grey.shade600, fontSize: 16)),
        const Text('Milk Business Portal', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
      ],
    );
  }
  ...
  Widget _buildVolumeGrid() {
    return GridView.count(
      ...
      children: [
        _buildStatCard('This Week', '${_data?['milk_week'] ?? 0} L', LucideIcons.calendar, Colors.blue),
        _buildStatCard('This Month', '${_data?['milk_month'] ?? 0} L', LucideIcons.layoutDashboard, Colors.indigo),
      ],
    );
  }
  ...
  Widget _buildRevenueCards() {
    ...
    final revenue = canSee ? (_data?['revenue_today']?.toString() ?? '0') : "---";
    final profit = canSee ? (_data?['profit_today']?.toString() ?? '0') : "---";
    final monthRev = canSee ? (_data?['revenue_month']?.toString() ?? '0') : "---";

    return Column(
      children: [
        _buildLongRevenueCard('Daily Revenue', 'RWF $revenue', 'RWF $profit', 'Profit', LucideIcons.coins, Colors.green),
        const SizedBox(height: 12),
        _buildLongRevenueCard('Monthly Revenue', 'RWF $monthRev', 'Active', 'Status', LucideIcons.trendingUp, Colors.purple),
      ],
    );
  }

  Widget _buildLongRevenueCard(String title, String value, String subValue, String subTitle, IconData icon, Color color) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        color: Colors.white,
        ...
        children: [
          ...
          Column(
            ...
            children: [
              ...
              Text(subValue, style: TextStyle(color: color, fontWeight: FontWeight.bold, fontSize: 14)),
            ],
          )
        ],
      ),
        ...
        child: Row(
          children: [
            Container(
              ...
              child: const Icon(LucideIcons.factory, color: Colors.blue, size: 20),
            ),
            const SizedBox(width: 16),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(site['name'], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                  Text('${site['collected']} L Today', style: TextStyle(color: Colors.blue.shade600, fontSize: 13, fontWeight: FontWeight.w500)),
                ],
              ),
            ),
  ...
  Widget _buildIconButton({required IconData icon, required int count, required VoidCallback onTap}) {
    return Stack(
      children: [
        IconButton(icon: Icon(icon, size: 22, color: const Color(0xFF64748B)), onPressed: onTap),
        if (count > 0)
          Positioned(
            right: 8, top: 8,
            child: Container(
